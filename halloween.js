...
function setupDeathNoteFeatures() {
  ...
  const setupRuleRotator = () => {
    ...
    const rulesBox = document.querySelector('.rules-box') || document.querySelector('.controls');
    if (!rulesBox) return;

    const ticker = document.createElement('div');
    ticker.id = 'dn-rule-ticker';
    ticker.className = 'rule-rotator-container';
    
    let currentIdx = 0;
    const initialRule = rulesList[0];
    
    ticker.innerHTML = `
      <span class="rule-ticker-label" id="dn-rule-label">RULE OF THE NOTE (I)</span>
      <p id="dn-rule-text" class="${initialRule.isFake ? 'fake-rule' : 'real-rule'}">${initialRule.text}</p>
    `;

    rulesBox.prepend(ticker);
